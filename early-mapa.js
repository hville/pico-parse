//https://github.com/aredridel/lotsawa/blob/master/index.js
/**
 *
 * @param {Array} rules
 * @returns
...
export function Grammar(rules) {
  // 'accept' rule means the entire grammar was accepted in the case of a nulling start symbol.
  rules.push( rules.acceptRule = Rule('_accept', [Ref('start')]) )

  // list all symbols in grammar to map to bit if present
	rules.symbols = []
	for (const r in rules) {
		if (symbolIndexOf(rules.symbols, r) === -1) rules.symbols.push(r)
		r.symbols.forEach( (s, i) => {
			let symNo = symbolIndexOf(rules.symbols, s)
			if (symNo === -1) {
				symNo = rules.symbols.length
				rules.symbols.push(s)
			}
			r.symbols[i] = symNo
		} )
		r.sym = symbolIndexOf(rules.symbols, r);
	}

	rules.by_symbol = []
	rules.forEach( (r, ruleNo) => {
		if (!rules.by_symbol[r.sym]) rules.by_symbol[r.sym] = []
		rules.by_symbol[r.sym].push(ruleNo)
	})
  ...
  function generateSymbolMatrix() {
    const predictable = makematrix(rules.symbols.length, rules.symbols.length);
  ...
  function generatePredictionMatrix() {
    const predictable = makematrix(rules.length, rules.length);
...
export function Rule(name, syms) {
  return {
    kind: 'Rule',
    name: name,
    symbols: syms
...
export function Terminal(symbol) {
  return {
    kind: 'Terminal',
    name: symbol,
    terminal: symbol,
...
export function parse(grammar, toParse, debug) {
...
export function Parser(grammar, debug) {
  const sets = [];
